...
- (void)updateNavigation {
    ...
    if (_gridController) {
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
            ...
            NSString *ss = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), NSLocalizedString(@"/", @"Used in the context: 'Showing 1 / 3 items'"), (unsigned long)numberOfPhotos];
            self.title = ss;
        }
    } else {
        self.title = nil;
